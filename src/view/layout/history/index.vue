...
export default {
  ...
  created() {
    ...
    // console.log(this.historys,this.$route)
  ...
  methods: {
    ...
    closeOther() {
      ...
      if (this.$route.name !== this.rightActive) {
        this.$router.push({ name: this.rightActive });
      }
      sessionStorage.setItem("historys", JSON.stringify(this.historys));
    },
    setTab(route) {
      if (
        !this.historys.some(item => item.name == route.name) &&
        route.name !== "layout"
      ) {
        const obj = {};
        obj.name = route.name;
    ...
    changeTab(tab) {
      if (this.activeValue !== this.$route.name) {
